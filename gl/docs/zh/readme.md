...
在图层标志左上角的不透明度百分比处输入数值/拖动/滚动可以调节图层的**不透明度**。在数值上右键单击或按住Shift键单击可以**隐藏/显示**图层。隐藏的图层将无法绘制，也无法移动。
...
左下角的<img src="../../resources/clip-mask.svg" height="16"/>按钮是**剪贴蒙版**按钮，点击以切换剪贴蒙版状态。剪贴蒙版图层只显示其下方普通图层的不透明部分对应的内容。
...
在一定需要打开大文件或操作大量图层的情况下，可以降低[渲染位深度]()通过牺牲一定的渲染质量来操作大文件或降低占用。一般而言，32位每通道才能保证不会出现任何色差或画笔波纹，但你可以根据自己的内容需求来调节这一选项。试试看！
