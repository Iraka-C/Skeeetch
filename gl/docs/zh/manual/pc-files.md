...
点击*文件* > *保存为PSD文件* 按钮可以将当前的画纸内容下载为一个PSD格式文件，也可以使用快捷键`Ctrl+Shift+S`。Skeeetch将尽力保留图层和图层组的内容及信息，但由于目前只支持低于Photoshop7.0格式的内容保存，一些选项可能无法被完全保留。如果想保留所有的选项可以使用`.skeeetch`格式，详见[导出和打开Skeeetch画纸](#导出和打开skeeetch画纸)。
