...
本节介绍纸张和库的管理。关于云文件管理，参见[云储存](./cloud.md)。
...
> | `Alt`      | 自由变换     | 自由变换 |
>
> 拖动选择框内部总是移动选择框。此时光标会变成抓手形状。按下`Shift`键拖动仍然是常规的平移画纸操作。
...
<img src="./images/file-drag.png" width="400"/>

Skeeetch支持常见的`.bmp/.jpg/.png/.tiff`等图片文件的置入。

将图片从文件管理器、其他网页、或者其他软件窗口中拖入Skeeetch的画纸即可置入文件。文件会被加载为当前画纸中**新的图层**。

也可以通过点击**打开文件**按钮从储存中选择，或者将图片拖入文件菜单（文件菜单中间会显示一个加号，如上图所示）。加载的图片将被打开为一个**新的文件**。
...
> 如果Skeeetch在加载文件时出现长时间无反应/网页崩溃等情况，可能是因为文件过大而耗尽浏览器资源。我们将不断优化Skeeetch以争取支持打开更大的文件，但一般而言，Skeeetch支持的文件大小和计算机配置、系统设置、浏览器都有关系。
...
<img src="./images/file-dump.png" width="300"/>

Skeeetch拥有自己的图片格式（`.skeeetch`文件）。在库中一个文件的图标上点击**下载按钮**（上图中有向下箭头的按钮）就可以下载这个Skeeetch画纸。这样就可以在其他时间地点重新打开画纸并继续你的工作。打开方式和打开`.psd`文件类似，可以将`.skeeetch`文件直接拖入窗口，或者从“打开文件”按钮选择文件。

也可以使用快捷键`Ctrl+Shift+S`来导出当前打开的画纸。

虽然`.psd`文件也可以保存画纸内容，并且可以被其他软件读取，`.skeeetch`文件相比`.psd`文件有一些优势：

* 同样的画纸保存为`.skeeetch`文件大小更小，平均只有`.psd`文件的一半大小左右。
* `.skeeetch`文件直接操作Skeeetch的数据库，因此无需像`.psd`文件一样的预渲染和编解码。不需要打开文件就可以直接从库列表中导出，打开和保存的速度也要快得多。
* `.skeeetch`文件可以保存Skeeetch支持的所有内容，而`.psd`文件有时不支持这些内容。

如果打开文件时Skeeetch发现该文件已经在库中有另一个版本，则会询问你如何操作。可以选择覆盖（只保留导入的文件）、取消（只保留已有的文件）、或者打开为新文件（均保留）。

> 注：文件版本与文件名无关。一个文件无论多少次修改、上传下载、重命名，均保持为同一文件。只有**创建新画纸**、**打开为新画纸**、或者**创建副本**会产生新的文件版本。
...
>
> <img src="./images/task-aborted.png" width="600"/>
>
> 以上是强行中止加载库中内容后的现象。为保护源文件而新建了一张显示"ERROR"的画纸。
...
点击*文件* > *保存为PSD文件* 按钮可以将当前的画纸内容下载为一个PSD格式文件。Skeeetch将尽力保留图层和图层组的内容及信息，但由于目前只支持低于Photoshop7.0格式的内容保存，一些选项可能无法被完全保留。如果想保留所有的选项可以使用`.skeeetch`格式，详见[导出和打开Skeeetch画纸](#导出和打开skeeetch画纸)。
...
此外，`Ctrl+Space+C`快捷键将复制所有可见的画纸到剪贴板。

`Ctrl+Space+V`快捷键将**粘贴并替换**当前图层（不支持图层组）的内容。
