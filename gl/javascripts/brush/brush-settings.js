...
BrushManager.initMenuOpacitySection=function(brushMenu) {
	...
	brushMenu.addSwitch(Lang("Pressure Controlled Eroding"),
		[Lang("Disabled"),Lang("erode-opacity")/*,Lang("erode-shape")*/],null,id => {
...
BrushManager.initScatterSetting=function(brushMenu) {
	...
	brushMenu.addInstantNumberItem(
		...
		() => {
			...
			return (brush.brushtip&&brush.blendMode!=2)||brush.isScatter>0? // allow customized interval
				(brush.interval*100).toFixed(brush.interval<0.1?1:0):NaN
		},"%",
		newVal => { // set on input
			if(newVal) {
				newVal=(newVal-0).clamp(0.01,1);
				BrushManager.activeBrush.interval=newVal;
			}
		},
		(dW,oldVal) => { // set on scroll
			const oldIntv=BrushManager.activeBrush.interval;
			let dIntv=oldIntv<0.02?0.001:oldIntv<0.05?0.005:0.01; // @TODO: tidy logic here
			let newVal=(oldIntv+dIntv*dW).clamp(0.01,1);
			BrushManager.activeBrush.interval=newVal;
		},
		(dx,oldVal) => { // set on drag-x
			oldVal=oldVal/100;
			let newVal=Math.exp(Math.log(oldVal)+dx/80).clamp(0.01,1);
			BrushManager.activeBrush.interval=newVal;
		}
	);
	brushMenu.addInstantNumberItem(
		...
		newVal => { // set on input
			if(newVal) {
				newVal=(newVal-0).clamp(0,3);
				BrushManager.activeBrush.scatRad=newVal;
			}
		},
		(dW,oldVal) => { // set on scroll
			oldVal-=0;
			let newVal=(oldVal+dW/20).clamp(0,3);
			BrushManager.activeBrush.scatRad=newVal;
		},
		(dx,oldVal) => { // set on drag-x
			oldVal-=0;
			let newVal=(oldVal+dx/100).clamp(0,3);
			BrushManager.activeBrush.scatRad=newVal;
		}
	);
	brushMenu.addInstantNumberItem(
